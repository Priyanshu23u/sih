  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //   axios.get("http://localhost:5000/api/final_news"),
        ...
        const newsData = newsResponse[0].data.map((item) => ({
          ...item, // Spread the original item properties
          disaster_type: item.disaster_type || "Unknown", // Provide default values
          location: item.location || "Unknown",
          date: item.date,
          short_description: item.short_description,
          category: getCategoryByDate(item.date), // Assuming this function is defined
        }));
        
        console.log(newsData);
        // const tweetData = tweetResponse[0].data.map((item) => ({
        //   ...item,
        //   _id: item._id,
